...
function findDuplicatesForAllUsers() {
  ...
  mysql.createConnection({
    host: "localhost",
    user: utils.secrets.Credentials.NextCloud.Db.UserName,
    password: utils.secrets.Credentials.NextCloud.Db.Password,
    port: 3306,
    database: "kummer"

  }).then((conn) => {
    logger.verbose("Opened DB connection");
    connection = conn;
    ...
    // Wait for all above to be finished
    Promise
      .all(allPromises)
      .then(results => {
        logger.verbose("Done waiting - results=%s", results);
    ...
    logger.verbose("Closing DB connection");
    connection.end();
...
function getAllCachedCalculations(connection, familyMemberSecrets, notes) {
  logger.verbose("In getAllCachedCalculations");
  return new Promise(resolve => {
    connection
      .query("SELECT * FROM find_duplicate_notes")
      .then(rows => {
        resolve(findDuplicateNotesForOneUser(connection, rows, familyMemberSecrets, notes));
      });
  });
}
